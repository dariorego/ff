...
class UaUQRSupabaseUser extends BaseAuthUser {
  UaUQRSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> uaUQRSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = UaUQRSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
